...
def GenerateLatinHyperCube(num, var_range):
    ...
    for range_ in var_range:
        lower_limit, step_size = np.linspace(range_[0], range_[1], num=num, retstep=True)
...
def GenerateRegularLattice(num, var_range):
    ...
    num = int(num ** (1./var_num) + 0.5)
    coord = []
    for index, range_ in enumerate(var_range):
        coord.append(np.repeat(np.linspace(range_[0], range_[1], endpoint=True, num=num).tolist()*(num**index), num**(var_num - index - 1)).tolist())
...
if __name__ == '__main__':
    ...
    print(content)
    df = pd.DataFrame(content, columns=['col%d' % num for num in range(0, content.shape[1])])
