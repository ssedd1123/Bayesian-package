...
def smoothed_histogram2D(x, y, ax, bins=20, sigma=0, nlevels=10, extent=None, **kwargs):
    if extent is not None:
        h, x, y = np.histogram2d(x, y, bins=bins, range=[[extent[0], extent[1]], [extent[2], extent[3]]])
    else:
    ...
    for i, row in enumerate(axes2d):
        for j, cell in enumerate(row):
            ...
            if namex == namey:
                ...
                smoothed_histogram1D(trace[namex], bins=bins, sigma=sigma, range=[prior['Min'][namex], prior['Max'][namex]], ax=dummy)
                dummy.yaxis.set_ticks([])
                dummy.set_xlim([prior['Min'][namex], prior['Max'][namex]])
            else:
                smoothed_histogram2D(trace[namex].to_numpy(), trace[namey].to_numpy(), bins=bins, sigma=sigma, ax=cell, cmap=cmap, 
                                     extent=[prior['Min'][namex], prior['Max'][namex], prior['Min'][namey], prior['Max'][namey]])
