...
def EarlyStopping(input_, target, validation_runs, 
                  initial_scales, initial_nuggets,
                  input_pipe, output_pipe, 
                  scales_rate=0.001, nuggets_rate=0.003, nsteps=1500):
    ...
    emulator = EmulatorMultiOutput(input_pipe.Transform(input_), output_pipe.Transform(target))
    ...
    for index, emu in enumerate(emulator.emulator_list):
        ...
        for i in range(nsteps):
            ...
            sys.stdout.write("\rProcessing %i iteration, likelihood = %f, nuggets = %f, scales = %s" % (i, val, nuggets, np.array2string(scales)))
            sys.stdout.flush()
